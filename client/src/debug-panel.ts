...
export class DebugPanel {
  ...
  private render(): void {
    ...
    this.content.innerHTML = html || '<div style="color: #888">No debug values</div>';
  }

  // Helper methods for common values
  setConnection(status: 'disconnected' | 'connecting' | 'connected', details?: string): void {
    const color = {
      disconnected: '#f44336',
      connecting: '#ff9800',
      connected: '#4caf50',
    ...
    this.set(
      'Connection',
      `<span style="color: ${color}">${status}</span>${details ? ` (${details})` : ''}`
    );
  }
