    ...
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(
            ...
            (
                ...
                formation_flocking_system.in_set(SpatialGridSet::Read),
                // Velocity sync must happen after flocking
                sync_group_boid_velocities.after(SpatialGridSet::Read),
            ),
        );
    }
}
    ...
    for (mut group, pos, mut velocity, lod) in groups.iter_mut() {
        ...
        let target_velocity = match &mut group.behavior_state {
            ...
            GroupBehavior::Engaging { primary_target, .. } => {
                ...
                if let Some(target_pos) = players
                    .iter()
...
fn formation_flocking_system(
    ...
    let mut groups_map: HashMap<Entity, Vec<(Entity, Vec2, Option<FormationSlot>)>> =
        HashMap::new();
    ...
    for (group_entity, members) in groups_map {
        if groups.get(group_entity).is_ok() {
...
fn calculate_full_flocking_forces(
    ...
    for &other_entity in neighbors {
        ...
        // Find in boid data
        if let Some((_, _, _, other_pos, other_vel, _)) = boid_data
            .iter()
            .find(|(e, _, _, _, _, _)| *e == other_entity)
        {
    ...
    // Wall avoidance
    let wall_force = calculate_wall_avoidance(position, config, game_config);
    ...
    for (obstacle_pos, obstacle) in obstacle_query.iter() {
        ...
        if distance < config.obstacle_avoidance_radius {
            ...
            if will_collide || distance < config.obstacle_danger_zone {
                ...
                let force_magnitude = config.obstacle_avoidance_weight
                    * (1.0 - (distance / config.obstacle_avoidance_radius));
...
fn calculate_player_avoidance(
    ...
    for (player_pos, _) in player_query.iter() {
        ...
        if distance < config.player_avoidance_radius && distance > 0.0 {
            ...
            // Stronger avoidance when closer
            let force_magnitude = config.player_avoidance_weight
                * (1.0 - (distance / config.player_avoidance_radius));
            avoidance_force += avoidance_direction * force_magnitude * config.max_speed;
        }
    }
