...
pub fn calculate_formation_positions(formation: &Formation, count: usize) -> Vec<Vec2> {
    match formation {
        ...
        Formation::SwarmAttack { spread, convergence_point } => {
            calculate_swarm_formation(count, *spread, *convergence_point)
        },
        Formation::PatrolLine { length, wave_amplitude } => {
...
fn update_formation_transitions(
    mut groups: Query<(&mut boid_wars_shared::BoidGroup, &boid_wars_shared::Position)>,
    players: Query<&boid_wars_shared::Position, With<boid_wars_shared::Player>>,
    time: Res<Time>,
) {
    for (mut group, pos) in groups.iter_mut() {
        let should_change_formation = match (&group.behavior_state, &group.current_formation) {
            (boid_wars_shared::GroupBehavior::Engaging { .. }, Formation::VFormation { .. }) => true,
            (boid_wars_shared::GroupBehavior::Defending { .. }, Formation::SwarmAttack { .. }) => true,
            (boid_wars_shared::GroupBehavior::Patrolling { .. }, Formation::CircleDefense { .. }) => true,
            _ => false,
        };
